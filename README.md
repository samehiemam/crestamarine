...
6. Set `APP_URL` to the public site origin, for example `https://example.com` (no trailing slash). In Google Cloud Console create an OAuth web client and add `https://YOUR-DOMAIN/api/auth/google/callback` as an authorised redirect URI. Add its client ID and secret to Hostinger.
